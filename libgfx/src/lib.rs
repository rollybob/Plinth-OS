...
/// 8x8 glyph cell dimensions, re-exported from `libfont` so callers that used
/// `libgfx::FONT_W` keep working unchanged.
pub use libfont::{FONT_H, FONT_W};

// The glyph table itself lives in `libfont`, shared with the kernel's
// diagnostic console (real_hardware.md D11) so there is one font rather than
// two. Only the DATA moved: deciding where a glyph lands, what colour it is and
// when to wrap is still this crate's job, and still library-OS policy.
use libfont::glyph;
